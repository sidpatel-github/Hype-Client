...
public class Updatepassemail extends Activity{

	public static String NAMESPACE = "http://tempuri.org/";
	public static String SOAP_ACTION = "http://tempuri.org/uppasswordemail";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.updatepassemail);
		...
		update.setOnClickListener(new View.OnClickListener() 
		{
			...
			public void onClick(View arg0) {
				if(!(npass.getText().toString().equals(cnpass.getText().toString())))
				 {
					 Toast.makeText(Updatepassemail.this,"password and confirm password must match", 2000).show();
				 }
				else if((npass.getText().toString().trim().length()==0) && (cnpass.getText().toString().trim().length()==0))
				{
					 Toast.makeText(Updatepassemail.this,"blank field", 2000).show();			
				}
				else if(!npass.getText().toString().matches(pattern))
				{
					 Toast.makeText(Updatepassemail.this,"password does not match pattern", 2000).show();			
				}
				else 
				{
					new AlertDialog.Builder(Updatepassemail.this)
	...
	public void onBackPressed() {
		        ...
		        .setPositiveButton("Yes", new DialogInterface.OnClickListener()
		    {
		        @Override
		        public void onClick(DialogInterface dialog, int which) {
		          /*  Intent i=new Intent(Updatepassemail.this,Login.class);
...
protected String doInBackground(Void... params) 
{
	try
	{			
		SoapObject request = new SoapObject("http://tempuri.org/","uppasswordemail");
	//	request.addProperty("username",user.getText().toString());
     ...
     //   request.addProperty("conpassword",cnpass.getText().toString());
        request.addProperty("email",Passotp.allemail);;
        
		SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
		envelope.dotNet=true;
		envelope.setOutputSoapObject(request);				
		HttpTransportSE androidHttpTransport = new HttpTransportSE(Login.URL);				
		androidHttpTransport.call("http://tempuri.org/uppasswordemail", envelope);				
...
{
	super.onPreExecute();
	pdia = new ProgressDialog(Updatepassemail.this);
...
protected void onPostExecute(String result) {
	...
	{
		Toast.makeText(Updatepassemail.this, "updated", 2).show();	
		finish();
		Intent i1 = new Intent(Updatepassemail.this,Login.class); 
		startActivity(i1);
	}
	else
	{
		Toast.makeText(Updatepassemail.this, "username wrong", 2).show();	
	}
}}
